...
/*
 * Reads shell PID from /proc/self/stat file
 * Returns PID as pid_t, or 0 on failure
 */
...
void init_shell(t_shell *shell, int argc, char **argv, char **envp) {
  ...
  shell->alias_table->buckets =
      gc_calloc(shell->global_arena, ENV_TABLE_SIZE, sizeof(t_env_bucket *));
